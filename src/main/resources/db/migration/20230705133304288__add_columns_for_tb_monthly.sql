...
('3efdf855-cc03-4ccc-97f2-60108bc4b7a3', 'REFERENCE_DATA');
...
SELECT
	t.id, a.id, a.definition, 7, a.indicator, false, a.label, a.name, NULL, 1, a.name, NULL
FROM
	...
	INNER JOIN requisition.available_requisition_columns AS a ON a.name = 'numberOfPatientsOnTreatmentNextMonth';

INSERT INTO
	requisition.columns_maps(requisitiontemplateid, requisitioncolumnid, definition, displayorder, indicator, isdisplayed, label, name, requisitioncolumnoptionid, source, key, tag)
SELECT
	t.id, a.id, a.definition, 8, a.indicator, false, a.label, a.name, NULL, 2, a.name, NULL
FROM
	requisition.requisition_templates AS t
	INNER JOIN (SELECT requisitiontemplateid, count(*) FROM requisition.columns_maps GROUP BY requisitiontemplateid) AS c ON c.requisitiontemplateid = t.id
	INNER JOIN requisition.available_requisition_columns AS a ON a.name = 'programOrderables.dosesperpatient';

INSERT INTO
	requisition.columns_maps(requisitiontemplateid, requisitioncolumnid, definition, displayorder, indicator, isdisplayed, label, name, requisitioncolumnoptionid, source, key, tag)
SELECT
	t.id, a.id, a.definition, 9, a.indicator, false, a.label, a.name, NULL, 1, a.name, NULL
FROM
	requisition.requisition_templates AS t
	INNER JOIN (SELECT requisitiontemplateid, count(*) FROM requisition.columns_maps GROUP BY requisitiontemplateid) AS c ON c.requisitiontemplateid = t.id
	INNER JOIN requisition.available_requisition_columns AS a ON a.name = 'totalRequirement';

INSERT INTO
	requisition.columns_maps(requisitiontemplateid, requisitioncolumnid, definition, displayorder, indicator, isdisplayed, label, name, requisitioncolumnoptionid, source, key, tag)
SELECT
	t.id, a.id, a.definition, 10, a.indicator, false, a.label, a.name, NULL, 1, a.name, NULL
FROM
	requisition.requisition_templates AS t
	INNER JOIN (SELECT requisitiontemplateid, count(*) FROM requisition.columns_maps GROUP BY requisitiontemplateid) AS c ON c.requisitiontemplateid = t.id
	INNER JOIN requisition.available_requisition_columns AS a ON a.name = 'totalQuantityNeededByHf';

INSERT INTO
	requisition.columns_maps(requisitiontemplateid, requisitioncolumnid, definition, displayorder, indicator, isdisplayed, label, name, requisitioncolumnoptionid, source, key, tag)
SELECT
	t.id, a.id, a.definition, 11, a.indicator, false, a.label, a.name, NULL, 1, a.name, NULL
FROM
	requisition.requisition_templates AS t
	INNER JOIN (SELECT requisitiontemplateid, count(*) FROM requisition.columns_maps GROUP BY requisitiontemplateid) AS c ON c.requisitiontemplateid = t.id
	INNER JOIN requisition.available_requisition_columns AS a ON a.name = 'quantityToIssue';

INSERT INTO
	requisition.columns_maps(requisitiontemplateid, requisitioncolumnid, definition, displayorder, indicator, isdisplayed, label, name, requisitioncolumnoptionid, source, key, tag)
SELECT
	t.id, a.id, a.definition, 12, a.indicator, false, a.label, a.name, NULL, 1, a.name, NULL
FROM
	requisition.requisition_templates AS t
	INNER JOIN (SELECT requisitiontemplateid, count(*) FROM requisition.columns_maps GROUP BY requisitiontemplateid) AS c ON c.requisitiontemplateid = t.id
	INNER JOIN requisition.available_requisition_columns AS a ON a.name = 'convertedQuantityToIssue';
