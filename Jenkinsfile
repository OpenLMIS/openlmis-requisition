        ...
        stage('Build') {
            steps {
                withCredentials([file(credentialsId: '8da5ba56-8ebb-4a6a-bdb5-43c9d0efb120', variable: 'ENV_FILE')]) {
                    ...
                    sh "docker tag openlmis/requisition:latest openlmis/requisition:${STAGING_VERSION}"
                    sh "docker push openlmis/requisition:${STAGING_VERSION}"
                }
            }
            post {
                success {
            ...
            steps {
                dir('erd') {
                    sh '''#!/bin/bash -xe
                        ...
                        && mkdir output \
                        && chmod 777 output \
                        && (docker run --rm --network ${COMPOSE_PROJECT_NAME//.}_default -v $WORKSPACE/erd/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s requisition -u postgres -p p@ssw0rd -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
                        && sleep 30 \
